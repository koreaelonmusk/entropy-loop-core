...
def test_html_has_reduced_motion_support() -> None:
    html = export_regression_triage_html(_triage())
    assert "prefers-reduced-motion" in html


def test_html_default_locale_is_english() -> None:
    html = export_regression_triage_html(_triage())
    assert 'lang="en"' in html
    assert "Entropy Loop Failure Console" in html


def test_html_korean_locale() -> None:
    html = export_regression_triage_html(_triage(), locale="ko")
    assert 'lang="ko"' in html
    assert "엔트로피 루프 실패 콘솔" in html
    assert "AI 에이전트 회귀를 CI 증거로 변환" in html


def test_html_korean_semantics_note() -> None:
    html = export_regression_triage_html(_triage(), locale="ko")
    assert "JUnit 실패는 리포트에 표시되는 회귀/테스트 상태를 의미" in html


def test_html_korean_is_deterministic() -> None:
    a = export_regression_triage_html(_triage(), locale="ko")
    b = export_regression_triage_html(_triage(), locale="ko")
    assert a == b
    assert "http://" not in a and "https://" not in a


def test_html_invalid_locale_raises() -> None:
    with pytest.raises(ValueError):
        export_regression_triage_html(_triage(), locale="de")


def test_html_writer_korean(tmp_path) -> None:
    path = tmp_path / "ko.html"
    write_regression_triage_html(_triage(), path, locale="ko")
    assert "엔트로피 루프 실패 콘솔" in path.read_text()
